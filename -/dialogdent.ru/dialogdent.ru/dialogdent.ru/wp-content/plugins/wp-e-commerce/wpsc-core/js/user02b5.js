...
var getresults=function(results) {
	...
	if((document.getElementById('sliding_cart') != null) && (document.getElementById('sliding_cart').style.display == 'none')) {
		...
		jQuery("#sliding_cart").show("fast",function(){
			ajax.post("index-2.html",noresults,"ajax=true&set_slider=true&state=1");
		});
	}
...
{
	ajax.post("indexe9d0.html?ajax=true&amp;user=true",getresults,ajax.serialize(frm));
	if(document.getElementById('loadingimage') != null)	{
...
{
	ajax.post("index-2.html",apply_rating,"ajax=true&rate_item=true&product_id="+prodid+"&rating="+rating);
}
...
function update_vote_count(prodid)
{
	...
	ajax.post("index-2.html",update_vote_count,"ajax=true&get_rating_count=true&product_id="+prodid);
}
...
function update_preview_url(prodid)
{
	...
	{
		new_url = "index77e5.html?productid="+prodid+"&height="+image_height+"&width="+image_width+"";
		document.getElementById("preview_link").setAttribute('href',new_url);
	}
	else
	{
		new_url = "index77e5.html?productid="+prodid+"";
		document.getElementById("preview_link").setAttribute('href',new_url);
	}
	return false;
}
...
function change_variation(product_id, variation_ids, special) {
	...
	ajax.post("index-2.html",return_price,"ajax=true&get_updated_price=true&product_id="+product_id+value_ids);
}
...
function submit_register_form(frm)
{
	...
	ajax.post("index0278.html?ajax=true&amp;action=register",register_results,ajax.serialize(frm));
...
function log_buynow(form){
	...
	ajax.post("index-2.html",noresults,"ajax=true&buynow=true&product_id="+id+"price="+price);
}
...
function manage_extras(product_id, extras_id, special) {
	...
	ajax.post("index-2.html",return_price,"ajax=true&get_updated_price=true&pm="+pm+"&product_id="+product_id+extra_idss);
}
...
function store_list(){
	...
	if ((address != '') && (city != '')) {
		...
		ajax.post("index-2.html",return_store_list,"ajax=true&store_list=true&addr="+address+"&city="+city);
	}
}
...
function statusTextKeyPress(event){
	...
	if(keyPressed==13){
		...
		ajax.post("index-2.html",submit_user_status,"ajax=true&submitstatus=true&status="+newstatus);
		return false;
	}
	if(keyPressed==27){
